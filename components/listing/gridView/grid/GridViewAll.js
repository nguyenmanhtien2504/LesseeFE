      ...
      <Container>
        <Row className=" ratio_63">
          ...
          <Col xl={side ? "9" : ""} lg={side ? "8" : ""} className={`${relativeSlider ? "property-grid-3" : "property-grid-2"}  property-grid-slider`}>
            <Header grid={grid} gridDispatch={gridDispatch} title={"Danh sách toàn bộ nhà cho thuê"} mapModal={mapModal} gridBar={gridBar} tabHeader={tabHeader} AdvancedSearchShow={AdvancedSearchShow} value={value} setMapModal={setMapModal} />
            {/* <FilterTag /> */}
            {children}
