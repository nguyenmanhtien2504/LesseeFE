...
const InputForm = ({ label, lg, sm, lastSm }) => {
  ...
  const handleClick = () => {
    router.push(`/listing/grid-view/map-modal/pigeon-mapSmall/?miA=${amountFilter[0]}&maA=${amountFilter[1]}&miD=${distanceFilter[0]}&maD=${distanceFilter[1]}&dh=${selectedOptions["Điều hòa"]}&nl=${selectedOptions["Nóng lạnh"]}&wf=${selectedOptions["Wifi"]}&mg=${selectedOptions["Máy giặt"]}&sd=${selectedOptions["Sân đỗ"]}&tl=${selectedOptions["Tủ Lạnh"]}&nb=${selectedOptions["Nhà Bếp"]}`);
  };
